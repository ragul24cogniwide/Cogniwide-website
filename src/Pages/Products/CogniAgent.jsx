...
const benefits = [
  ...
  const useCases = [
  {
    ...
    desc: "Automate loan application processing, fraud detection, and compliance checks.",
    tags: [
      { text: "Loan Processing", color: "bg-red-400" },
      { text: "Automated Fraud Detection", color: "bg-green-400" }
    ]
  },
  {
    ...
    icon: (
      <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-blue-400">
        <svg className="w-7 h-7 text-white" viewBox="0 0 24 24" fill="none">
          ...
          <path d="M12 10v4M9 14h6" stroke="currentColor" strokeWidth="2"/>
        </svg>
      </div>
    ),
    title: "Healthcare",
    desc: "Streamline patient record analysis, claims processing, and medical coding.",
    tags: [
      { text: "Claims Processing", color: "bg-red-400" },
      { text: "Patient Data Extraction", color: "bg-green-400" }
    ]
  },
  {
    ...
    title: "Manufacturing",
    desc: "Optimize supply chain logistics, quality control, and predictive maintenance.",
    tags: [
      { text: "Supply Chain Bottlenecks", color: "bg-red-400" },
      { text: "Automated Quality Checks", color: "bg-green-400" }
    ]
  }
];
...
// Box icons
const boxIcons = [
  <svg width="28" height="28" fill="none"><rect width="28" height="28" rx="6" fill="#fff"/><path d="M7 15l7-7 7 7" stroke="#3b82f6" strokeWidth="2"/></svg>,
  <svg width="28" height="28" fill="none"><rect width="28" height="28" rx="6" fill="#fff"/><path d="M14 7v14M7 14h14" stroke="#10b981" strokeWidth="2"/></svg>,
  <svg width="28" height="28" fill="none"><rect width="28" height="28" rx="6" fill="#fff"/><path d="M7 21l7-7 7 7M7 7h14" stroke="#ec4899" strokeWidth="2"/></svg>,
];

const featureBoxes = [
  {
    title: 'Flexible Framework',
    desc: 'Our robust and extensible framework provides the foundation for building highly specialized AI agents.',
  },
  {
    title: 'Developer-Friendly Tools',
    desc: 'Access comprehensive SDKs, APIs, and documentation to accelerate your custom agent development.',
  },
  {
    title: 'Expert Support',
    desc: 'Our team of AI specialists is available to provide guidance and support throughout your development journey.',
  },
];
...
const deployments = [
  {
    ...
    icon: (
      <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-cyan-400">
        <svg className="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24">
          <path d="M17 17a5 5 0 0 0-9.9-1M13 6A5 5 0 0 1 17 17H7a5 5 0 0 1 3-11z" stroke="currentColor" strokeWidth="2" />
        </svg>
      </div>
    ),
    title: "Cloud Deployment",
    desc: "Leverage the scalability and flexibility of cloud infrastructure for rapid setup and global access.",
    bullets: [
      "Managed service",
      "Elastic scalability",
  ...
  },
  {
    ...
    desc: "Maintain full control over your data and infrastructure, meeting strict regulatory and security requirements.",
    bullets: [
  ...
  },
  {
    ...
    icon: (
      <div className="w-12 h-12 rounded-xl flex items-center justify-center bg-pink-400">
        <svg className="w-7 h-7 text-white" fill="none" viewBox="0 0 24 24">
          <path d="M9 15l-6 6M15 9l6-6M12 12a9 9 0 0 1 6.36 2.64L21 21" stroke="currentColor" strokeWidth="2"/>
        </svg>
      </div>
    ),
    title: "Hybrid Deployment",
    desc: "Combine the best of both worlds, optimizing for performance, cost, and data residency needs.",
    bullets: [
      ...
      <div className="max-w-7xl w-full mx-auto px-4 flex flex-col lg:flex-row gap-12 items-center">
        ...
        <div className="flex-1 w-full flex flex-col items-center gap-8 relative">
          ...
          <img
            src={bot}
            alt="Agent Dashboard"
            className="w-40 h-auto rounded-xl bg-transparent object-contain absolute -top-20 right-[0px] hidden xl:block"
          />
        </div>
      </div>
    </section>
...
{/* Section4 */}
<section className="w-full py-16 bg-background fade-in md:-mt-40">
      <div className="max-w-7xl mx-auto px-4">
        <h2 className="text-3xl md:text-4xl font-bold text-center mb-3 text-purpletext">
          Transformative Use Cases Across Industries
        </h2>
        <p className="text-lg text-gray-500 text-center mb-10 max-w-3xl mx-auto">
          See how CogniAgent delivers measurable impact in various sectors.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {useCases.map((item, idx) => (
            <div
              key={item.title}
              className={`
                bg-white rounded-2xl p-6 shadow-md flex flex-col items-start
                border transition-all duration-300
                ${highlighted === idx ? "border-purple text-purple shadow-purple/40" : "border-transparent text-white"}
                hover:border-purple hover:text-purple hover:shadow-purple/40 hover:scale-105
                focus:outline-none fade-in zoom-in
            ...
            >
              ...
              <div className={`text-[15px] text-gray-300 mb-4 ${highlighted === idx ? "text-purple" : "text-gray-500"}`}>
                {item.desc}
              </div>
              <div className="flex flex-wrap gap-2 pt-2">
                {item.tags.map((tag, tidx) => (
                  <span
                    key={tidx}
                    className={`px-3 py-1 rounded-full text-xs font-semibold text-white ${tag.color} shadow`}
                  >
                    {tag.text}
                  </span>
                ))}
    ...
    {/* section5 */}
 <section className="bg-background min-h-screen w-full flex flex-col items-center py-8 px-2">
      <div className="max-w-5xl w-full">
        {/* Title */}
        <h1 className="text-purpletext font-bold text-2xl sm:text-3xl mb-2 text-center">
          Build Your Own: Custom Agent Development
        </h1>
        <p className="text-gray-500 text-base text-center mb-8">
          Leverage CogniAgent’s flexible framework to create bespoke AI agents tailored to your unique needs.
        </p>
        <div className="flex flex-col lg:flex-row w-full gap-6 justify-center items-start">
          {/* Feature Boxes */}
          <div className="flex flex-col gap-4 w-full lg:w-2/3">
            {featureBoxes.map((box, idx) => (
              <div
                key={box.title}
                className={`flex items-center gap-4 rounded-xl p-4 transition-transform duration-300
                  ${boxColors[idx]} 
                  ${hoveredIdx === idx ? 'scale-105 shadow-lg text-purple-500' : 'shadow-md text-gray-500'}
                  cursor-pointer
                  w-full sm:w-[330px]
                  mx-auto
                  `}
              ...
              >
                <div className="flex-shrink-0">{boxIcons[idx]}</div>
                <div>
                  <div className="font-bold mb-1">{box.title}</div>
                  <div className="text-sm">{box.desc}</div>
                </div>
              </div>
            ))}
          </div>
          ...
          <div className="w-full lg:w-1/2">
            <div className="rounded-xl bg-gray-900 text-white text-xs font-mono shadow-md p-4 lg:mt-0 mt-6 transition-transform duration-300 hover:scale-105">
              <div className="text-cyan-400 mb-2">// Example: Define a custom invoice validation agent</div>
              <pre className="whitespace-pre-wrap">
{`class CustomInvoiceAgent(CogniAgent):
    def __init__(self):
        super().__init__("invoice_validator")
    def process(self, invoice_data):
        # Implement custom validation logic here
        if invoice_data.total > 10000:
            return {"status": "flagged", "reason": "High value invoice"}
        return {"status": "approved", "reason": "Validation successful"}

// Instantiate and use the agent
agent = CustomInvoiceAgent()
result = agent.process({"total": 12000, "items": [...]})
print(result)`}
              </pre>
            </div>
          </div>
    ...
    <section className="w-full py-16 bg-background fade-in md:-mt-40">
      <div className="max-w-7xl mx-auto px-4">
        ...
        <p className="text-lg text-gray-500 text-center mb-10 max-w-3xl mx-auto">
          Choose the deployment option that best fits your infrastructure and security requirements.
        </p>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {deployments.map((item, idx) => (
            <div
              ...
              className={`
                bg-white rounded-2xl p-6 shadow-md flex flex-col items-start
                border transition-all duration-300
                ${highlighted === idx ? "border-purple text-purple shadow-purple/40" : "border-transparent text-black"}
                hover:border-purple hover:text-purple hover:shadow-purple/40 hover:scale-105
                focus:outline-none fade-in zoom-in
              `}
              onMouseEnter={() => setHighlighted(idx)}
            ...
            >
              {item.icon}
              <div className={`font-bold text-base mt-3 mb-2 ${highlighted === idx ? "text-purple" : "text-black"}`}>
                {item.title}
              </div>
              <div className={`text-[15px] text-gray-700 mb-4 ${highlighted === idx ? "text-purple" : ""}`}>
                {item.desc}
              </div>
              <ul className="space-y-1">
                {item.bullets.map((b, bidx) => (
                  <li
                    key={bidx}
                    className={`pl-4 relative before:absolute before:left-0 before:top-2 before:w-2 before:h-2 before:rounded-full
                      before:${item.color} text-sm ${highlighted === idx ? "text-purple" : "text-gray-700"}`}
                  >
                    {b}
                  </li>
                ))}
        ...
        <div className="mt-16 text-center fade-in">
          <h3 className="text-2xl md:text-3xl font-bold mb-4 text-gray-900">
            Ready to <span className="text-purpletext">Automate with AI Agents?</span>
          </h3>
          <p className="mb-8 text-lg text-gray-400">
            Discover how CogniAgent can streamline your business processes and drive efficiency.
          </p>
          <div className="flex flex-col sm:flex-row items-center justify-center gap-4">
            <button
              className="px-6 py-2 font-semibold text-base rounded-md bg-transparent border-2 border-cyan-400 text-cyan-400 shadow-cyan-400 shadow focus:outline-none ring-1 ring-cyan-400 transition
                hover:text-purple hover:border-purple hover:shadow-purple"
            >
              Get Started with CogniAgent →
            </button>
            <button
